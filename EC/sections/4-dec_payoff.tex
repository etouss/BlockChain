...
can be used to defeat the default strategy. \textbf{Juan: has any other model done this? cite!}
\textbf{Juan: write this and see what happens} We finish with some remarks about obtaining a nash equilibrium under decreasing fees. 
...
proof of this theorem (appendix \ref{sec-evaluation-G}), involving two sets
of polynomials related to Dyck words, which describe the combinatorial nature of
the game.

\paragraph{Outline of the proof.} First, consider the beginning of the game,
i.e., where both players are mining on $\varepsilon$. Miner 1, playing with
$G_\ell^k$, can obtain positive reward from five types of arising states. In the
...
cases are given when Miner 0 appends $j>k$ blocks on top of
$\varepsilon$, after which Miner 1 forks on block $j-k$ and wins (respectively,
looses) the fork.

        \end{enumerate}
\end{enumerate}

Because how these strategies are defined, after each of these states, the
behaviour repeats but replacing $\varepsilon$ by a later block $b$. Therefore,
we again have an equation of the form
$$u_1(\bgup^k_\ell)=\Phi+\Gamma\cdot u_1(\bgup^k_\ell),$$
where $\Phi$ is the contribution of the above states, and $\Gamma$ is a proper
...
represent states as unitary north-east paths inside polygons in $\ZZ^2$. In our
...
\begin{lemma}
 ...
 trapezoid $\{(0,0),(a,0),(a+b,b),(0,b)\}.$ We have
$$ 
    \displaystyle \sigma_{a,b}=\frac{(a+b)\cdot(a+2b)!}{b!\cdot(a+b+1)!}=\frac{a+1}{a+b+1}\cdot {a+2b\choose a+b}.
    $$
    Also, let $\theta_{a,b}^r$ be the
 amount of such paths from $(0,0)$ to $(r,b)$ that do not exit the pentagon
 $(0,0),(a,0),(r,r-a),(r,b),(0,b)$. We have the following
 $$
 \begin{cases}
