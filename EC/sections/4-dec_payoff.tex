...
In order to determine whether there is an incentive for player~1 to fork, we first need to determine her utility when she is playing according to the default strategy $\cdf = (\df_0,\df_1)$. 
...
\begin{tikzpicture}[->,>=stealth',auto,thick, scale = 0.61,state/.style={circle,inner sep=2pt}]
	...
	\node [state] at (-16.5,-1.4) (aG) {$\gup_4^2$};
...
%this last block 
$0000$ was the genesis block. Note that in fact $\af=\gup_\infty^\infty$, that is, the strategy where player $1$ is willing to keep fighting for a fork of any length, and to do a fork at the beginning of any chain of blocks she does not own.  \qed
\end{example}
...
\begin{figure*}
    ...
    \hfill
    \subfigure[Zoom in around the values of the hash power where $\bdf$ intersects with $\bgup^1_4$ and $\bgup^1_5$.
]{
        \input{../plots/forking_strategies_zoom_chart.tex}
        \label{fig-af-df-fo}
    }
...
%(appendix \ref{sec-evaluation-G}),
 involving two sets
of polynomials related to Dyck words, which describe the combinatorial nature of
the game.

{\sc Proof sketch.} First, consider the beginning of the game,
i.e., where  players are mining on $\varepsilon$. Miner 1, playing with
$G_\ell^k$, can obtain positive reward from five types of states. In the
first case, she immediately puts a block in top of $\varepsilon$. The second and
third cases are those in which miner 0 appends $j\leq k$ blocks on top of
$\varepsilon$ and miner 1 starts a fork on $\varepsilon$, eventually winning (respectively, loosing). The last two
cases are occur  when miner 0 appends $j>k$ blocks on top of
$\varepsilon$, after which miner 1 forks on  the block $j-k$ and wins (respectively,
looses) the fork.
\marcelo{I don't understand the use of "(respectively, looses)" in this paragraph. Isn't miner 1 supposed to have a positive reward?}
%        \end{enumerate}
%\end{enumerate}

Because of how these strategies are defined, after each of the states mentioned in the previous paragraph, the
behaviour repeats but replacing $\varepsilon$ by a later block $b$. Therefore,
as in the case of Theorem \ref{thm:always_fork}, we have an equation of the form
%\begin{eqnarray*}
$u_1(\bgup^k_\ell)  =  \Phi_{\ell, k}+\Gamma_{\ell, k}\cdot u_1(\bgup^k_\ell)$,
%\\end{eqnarray*}
where $\Phi_{\ell, k}$ is the contribution of the states mentioned in the previous paragraph, and $\Gamma_{\ell, k}$ is a proper
shifting factor. In order to obtain these quantities, we need to describe the
probabilities and combinatorics of the above states. To do this, we again
represent states as unitary north-east paths inside polygons in $\ZZ^2$. \marcelo{As far as I can see, this is the first time we mention north-east paths. We could introduce them in the proof of  Theorem \ref{thm:always_fork}, or we could explain them here. I like the first alternative as this shows that we are using the same idea in both proofs, although the combinatorics get more complicated in this case.}
In our
case, the pair of strategies imply that all paths stay within pentagons,
moreover, we establish the following combinatorial results:
\marcelo{The sentence "In our
case, the pair of strategies imply that all paths stay within pentagons" is not clear. We need a longer explanation showing why this type of paths is used in the proof.}
\begin{lemma}
   For nonnegative integers $a,b,r$, let $\sigma_{a,b}$ be the amount of
 one-step north-east paths from $(0,0)$ to $(a+b,b)$ that stay inside of the
 trapezoid $\{(0,0),(a,0),(0,b),(a+b,b)\}.$ \marcelo{Why is $r$ mentioned here? As far as I can see, $r$ is not used in the following equation.} We have that
\begin{eqnarray*}
\sigma_{a,b} & = & \frac{(a+b)\cdot(a+2b)!}{b!\cdot(a+b+1)!}=\frac{a+1}{a+b+1}\cdot {a+2b\choose a+b}.
\end{eqnarray*}
    Also, let $\theta_{a,b}^r$ be the
...
\theta_{a,b}^r & = & 
 ...
 \end{eqnarray*}
 \end{lemma}
As it turns out, the utility $u(\bgup_\ell^k)$ in this case is given as a
combination of the generating polynomials related to these sequences, which we
refer to as \textit{Trapezoidal and Pentagonal Dyck polynomials}, completing the
proof. \marcelo{It is not clear how the lemma and the coefficients $\theta_{a,b}^r$ are used in the proof. Please explain (we don't need to include every details, just an explanation that gives an idea of how this lemma is used and why the coefficients $\theta_{a,b}^r$ are important for the proof).} 
\qed

\medskip

%In other words, given specific values of $k$ and $\ell$, we can still compute the utility of these strategies.
We use this theorem to analyse these strategies, plotting them, as we did before, 
...
the more hash power the bigger the window of blocks one should aim to do a fork, and the more time one should wait before giving up. We notice that the analysis 
