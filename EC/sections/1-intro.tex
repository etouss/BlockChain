...
\smallskip
\noindent{\bf Contributions.}  Our first contribution is to provide a model of mining as an infinite stochastic game, in which maximising the utility corresponds to both 
putting block in the blockchain and maintaining them there for as long as possible. One of the benefits of our model is that using a few basic design parameters we can in fact accommodate different cryptocurrencies, and not focus solely on Bitcoin. These parameters also allow us to account for fundamental factors such as deflation, or discount in the block reward. Another benefit of our proposal is the fact that we can reason about all possible mining strategies, without the need to focus on a specific subclass. 
...
as our model imposes much stronger assumptions against forking than 
