...
\item $\suc: \bB \times \bP \to \bB$: This function tells us which will a player $p$ use to extend the current blockchain when mining on top of a block $b$. We require this function to be injective and if $\suc(b_1,p)=b_2$, then $\owner(b_2)=p$.
\end{itemize}
...
\begin{eqnarray*}
N &=& q\\
E &=& \{ (b_1,b_2) \in q^2 \mid \exists p \text{ s.t. }\suc(b_1,p) = b_2\}
\end{eqnarray*}
...
\end{itemize}
In a full disclosure scenario all the allowed actions are of the form $\mine(p,b,q,q \cup \{\suc(b,p)\})$, that is, all the information of player $p$ after placing $b$ in its body of knowledge is sent to the other players. In this case, we simplify the notation and use $\mine(p,b,q)$ instead of $\mine(p,b,q,q\cup \{\suc(b,p)\})$.
