...
\end{eqnarray*}
\francisco{In the latter definition, I think we have to include some conditions on $p,b,q,q'$... if for no better reason, they are not defined in the scope of the set definition.}
...
\begin{eqnarray*}
u_p^n(\bs \mid \bq_0) & = & \beta^{|\bq_0|} \cdot \sum_{i=0}^{n}\beta^{i} \cdot  \bigg(\sum_{\bq \in \bQ \,:\, |\bq| = i} r_p(\bq) \cdot 
...
\begin{eqnarray*}
u_p(\bs \mid \bq_0) & = & \beta^{|\bq_0|} \cdot \sum_{i=0}^{\infty}\beta^{i} \cdot  \bigg(\sum_{\bq \in \bQ \,:\, |\bq| = i} r_p(\bq) \cdot 
\pr^{\bs}(\bq \mid \bq_0)\bigg)
\end{eqnarray*}
\end{mydef}
