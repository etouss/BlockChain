...
A stationary strategy for a player $p$ a function $s : \cQ \rightarrow \cA_p$. Moreover $\cS_p$ is the set of all strategy for player $p$, and $\cS = \cS_{1} \times \cS_{2} \times \cdots \times \cS_{n}$ is the set of strategies for the game. Thus, every element $\bs$ of $\cS$ is a tuple containing exactly one strategy for each player. Finally, assuming that $\bs = (s_1, \ldots, s_n)$ and $\bq \in \cQ$, define $\bs(\bq)$ as the tuple of actions $(s_1(\bq), \ldots, s_n(\bq))$. 

Given an initial state $\bq_0 \in \cQ$ and a strategy $\bs \in \cS$, the probability of reaching state $\bq \in \cQ$ in $k$ iterations is recursively defined as follows:
\begin{eqnarray*}
\pr_0^{\bs}(\bq \mid \bq_0) & = &
\begin{cases}
1 & \text{if } \bq = \bq_0\\
