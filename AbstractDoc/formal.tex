...
\subsection{Knowledge trees}

\marcelo{We need to add the condition that $K$ is prefix closed}
...
\begin{mydef}
	...
	Then a maximal element of $K$ with respect to $\preceq_t$ is said to be a blockchain of $K$ at iteration $t$ with respect to the protocol $P$.
\end{mydef}
...
%We are dealing with equivalence knowledge in order to reduce the number of action possible.
