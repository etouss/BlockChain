...
For the sake of contradiction assume $u_1(\bdf) \leq u_1(\baf)$, and that there exists $j > 0$ with $u_1((\df_0,\pf{j})) \leq u_1(\af))$. 
...
\tau(q) & = & \{ b_\rho \mid \rho \text{ is a sequence for } q\}.
\end{eqnarray*}
And we trivially obtain the same properties, especially for every distinct words $w \neq w' \in \{0, 1\} ^*$, there exist two unique distinct states $q \neq q' \in Q_\pf{j}$,  such that $w \in \tau(q)$ and $w' \in \tau(q')$. %Moreover there also exits a state $q'' \in Q_\pf{j}$, such that $w\cdot w' \in \tau(q'')$ and we have $\pr^\pf{j}(q'') = \pr^\pf{j}(q) \cdot \pr^\pf{j}(q''\mid q)$.
After having performed $j$ successful fork, player $1$ is playing default strategy. As seen in proof of Theorem \ref{}, a successful can fork is represented by a word of the work $d\cdot 1$ where $d \in \Dyck$. Extending the pattern, the states reach immediately after $j$ successfully fork are represented by word of the form $d_1 \cdot 1^+ \cdot d_2 \cdot 1^+ \cdots d_j \cdot 1$ where $(d_1, \cdots, d_j) \in \Dyck^j$. We denote the set of word of this form 
