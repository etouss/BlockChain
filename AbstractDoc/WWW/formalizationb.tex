...
In Section \ref{sec-pay-uti}, we discuss the difficulty of defining a reward function which follow this rules, and therefore propose a pay-off model that mimic those incentives. For now we assume, for each player $p \in \bP$,  the existence of a reward function $r_p : \bQ \to \mathbb{R}$ such that the reward of $p$ in a state $q$ is given by $r_p(q)$. Moreover, the combined reward function of the game is $\bR = (r_0, r_1, \ldots, r_{m-1})$. 
