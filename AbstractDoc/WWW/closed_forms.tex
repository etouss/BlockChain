...
\section{Evaluation of $u(\gup^k_\ell)$}
...
We have
\begin{align*}
u_{a,j}(h) &= (1-\beta) \cdot \sum_{w\in \bstring, d\in \Dyck_{j-1}, |d| \leq 2\ell-j}\beta^{|d|+j+1+|w|}\cdot \bigg(\sum_{i=0}^{(|d|+j)/2}\alpha^{i+1} + \alpha^{(|d|+j)/2 + 1} r(w)\bigg)\cdot h^{(|d|+j)/2 + 1}\cdot (1-h)^{(|d|+j)/2}\cdot \pr(w).
\end{align*}
Simplifying this expression yields
\begin{align*}
u_{a,j}(h) &= 
\frac{\alpha\cdot\beta\cdot h}{1-\alpha}\cdot x^j \cdot 
\bigg(\sum_{d\in \Dyck_{j-1}, |d|\leq 2l - j}x^{(|d|-j)/2}\cdot (1-\alpha^{(|d|+j)/2 + 1})\bigg) \\
& +
\alpha\cdot\beta\cdot h \cdot(\alpha\cdot x)^j\cdot \bigg(\sum_{d\in \Dyck_{j-1}, |d|\leq 2l - j} (\alpha\cdot x)^{(|d|-j)/2}\bigg)\cdot\bigg(\sum_{w\in\bstring} \beta^{|w|}\cdot r_1(w)\cdot \pr(w)\bigg)\\
