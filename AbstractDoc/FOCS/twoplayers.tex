...
total utility amounts to $$u_p(\bs \mid q_0) = c\cdot h \cdot \sum_{i=0}^{n}i \cdot \beta^{i}$$ (recall that $q_0$ is the genesis block). 
...
\section{Alternative game definitions}
...
% {\bf $\alpha$-discounted $k$-block reward.} The pay-off of a player $p \in \bP$ is defined as:
%\begin{multline*}
%r_p(q) \ = \
%{\displaystyle c \cdot \bigg(
%\sum_{i=1}^{\lfloor\frac{|\meet(q)|}{k}\rfloor-1}
%\alpha^i \cdot \sum_{j=i \cdot k}^{(i+1) \cdot k -1} \chi_p(\meet(q),j)
%\ +}\\
%{\displaystyle \alpha^{\lfloor\frac{|\meet(q)|}{k}\rfloor} \cdot \sum_{j= \lfloor\frac{|\meet(q)|}{k}\rfloor \cdot k + 1}^{|\meet(q)|}  \chi_p(\meet(q),j)\bigg)}
%\end{multline*}
