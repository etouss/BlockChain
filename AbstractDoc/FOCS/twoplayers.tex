...
\pr^{\df}(w \mid \varepsilon)\bigg)\\
							& = & c\cdot \sum_{i=0}^{n}\beta^{i}\sum_{j=1}^{i} \alpha^j \expected(w[j]) = ch\cdot \sum_{i=0}^{n}\beta^{i}\sum_{j=0}^{i} \alpha^j 
\end{eqnarray*}
...
\end{eqnarray*}
It is widely known that $\Dyck_{2k}=C_k$, where $C_k$ is the $k$-th Catalan number, yielding the result. \francisco{TODO: Exact constants, $n=\infty$ case, references to Catalan}
\end{proof}


\subsection{Utility of the $k$-fork strategy}

In the same strategy framework as section \ref{sec-genfork}, we now suppose that player 1 attempts to fork once, but instead of mining upon the genesis block, she only goes back $k$ blocks on an already defined blockchain of arbitrary length. In other words, let $c$ be the current blockchain of length $|c|\geq k$, and let $b_{-k}$ the block at position $|c|-k$. We also suppose that $\owner(b_{-k+i})=2$ for all $i=1,\dots,k$. Also, for the sake of simplicity and without loss of generality, in the computation of utility we don't count any reward given at blocks before $b_{-k}$ (these rewards acting as additive constants). Let us define the following.


\begin{mydef}
	A Dyck draw of disadvantage $k$ is a binary string $d\in\{0,1\}^\ast$ such that $\Delta(d)+k=0$ and every initial substring $d'$ of $d$ verifies $\Delta(d')+k\leq 0$. We denote by $\Dyck_{2n,k}$ the set of Dyck draws of disadvantage $k$ of length $2n+k$ and $\Dyck^\ast_{k}$ the set of all Dyck draws of disadvantage $k$.
\end{mydef}

Now, as before note that every state of positive reward for player 1 is a state that successfully orphaned blocks from the original chain. The binary encoding can be written as $d1w$, where $d$ is a Dyck draw with disadvantage $k$ and $w\in\{0,1\}^\ast$ represent default playing by both players after the fork succeeds.
