...
%actions, and we restrict the set $\bQ$ of states to be the smallest set satisfying that:
%\begin{itemize}
...
\end{itemize}
\etienne{Sorry but the second item is wrong it could be any block up to $longest(q,1)$ which might not be the last (both according to definition of greedy and to what we need) Informally a player could stop trying to push is previous fork forward, because he has a too low probability to succeed but start a new fork further away on the longest chain of the other player but not on the last block. (As long at it further away)}
