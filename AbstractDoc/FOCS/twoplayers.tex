...
Given $p \in \{0,1\}$, $b \in \bB$ and a state $q$, an action $\mine(p,b,q)$ is {\em greedy} if $\mine(p,b,q)$ is a valid action and $\length(q,p) \leq |b|$.
\end{mydef}
...
\fg_p(q) & = &
\begin{cases}
\mine(p,\varepsilon,q) & \text{if } p \not\in q \text{ and } \longest(q,p) = \varepsilon\\
\df_p(q) & \text{if } p \not\in q \text{ and } \longest(q,p) \neq \varepsilon\\
