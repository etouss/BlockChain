...
\end{eqnarray*}
In this section, we show that when miners' fees decrease over time, forking can be a good strategy. We explore in Sections~\ref{sec-forkingstrategies} and \ref{sec-giving-up} several strategies based on the maximum length of the fight the miner is willing to carry out when doing a fork, and how far back the miner is willing to do a fork. %Interestingly, for such strategies 
Not only we confirm the folklore fact that it is profitable to fork with more than half of the hash power, but our exploration in Section \ref{sec-giving-up} gives us a concrete strategy that 
beats the default strategy with less than half of the hash power.
